...
#        weight_shifted = shift_convolve(weight_g, shift)#not modified
    ...
    def step(time_idx,lstm_hidden,input_hidden):
...
def build(P, input_size, mem_width, ctrl):
    step = build_step(
        P, ctrl, input_size, mem_width)

    def predict(input_hidden, lstm_output_gate):
        ...
        outputs , _ = theano.scan(
            ...
            sequences=time_idx,
            outputs_info = None,
            non_sequences=[lstm_output_gate,input_hidden]
        )
        return outputs
    return predict
