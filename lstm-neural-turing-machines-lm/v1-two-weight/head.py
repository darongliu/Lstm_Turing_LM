...
#    P["W_%d_key" % id] = U.initial_weights(input_size, mem_width)
    ...
    P["W_%d_att" % id] = U.initial_weights(input_size, mem_width)
    P["b_%d_att" % id] = 0. * U.initial_weights(mem_width)
...
#    P["b_%d_shift" % id] = 0. * U.initial_weights(shift_width)
    ...
    def head_params(x):
        ...
        #attention  weight
        att_t = T.nnet.sigmoid(T.dot(x, P["W_%d_att" % id]) + P["b_%d_att" % id])
