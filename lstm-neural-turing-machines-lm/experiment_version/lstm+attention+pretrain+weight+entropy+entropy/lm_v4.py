...
def parse_args(parser):
	...
	parser.add_argument('--key-entropy-reg', action="store", dest='key_entropy_reg',
						help='key entropy regulizer, default: 0.001 ', type=float, default=0.001)
...
#		print "done compile"
	...
	def create_final_prediction(self, greedy=False):
		...
		key_entropy_result = temp[2]
		#the last layer do transpose before compute
		return turing_result.transpose((1,0,2)),entropy_result.transpose((1,0)),key_entropy_result.transpose((1,0))
		# we reorder the predictions to be:
		# 1. what row / example
		# 2. what timestep
		# 3. softmax dimension	
...
def training(args, vocab, train_data, train_lengths, valid_data, valid_lengths):
	...
	print 'key entropy reg: %f' % args.key_entropy_reg
	...
	model = Model(
		...
		key_entropy_reg = args.key_entropy_reg,
