...
#        ][::-1]
    ...
    heads = [head.build(P, h, mem_width, mem_width/2) for h in range(no_heads)]

    def build_read(M_curr, weight_curr):
        return T.batched_dot(weight_curr, M_curr[:,:,mem_width/2:])
...
#        return T.batched_dot(shift , weight[:,shift_conv])
    ...
    def build_head_curr(weight_prev, M_curr, head, input_curr):
        ...
        weight_c = T.nnet.softmax(similarity(key, M_curr[:,:,:mem_width/2]))
