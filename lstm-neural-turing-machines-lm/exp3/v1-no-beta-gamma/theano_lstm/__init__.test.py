...
class Layer(object):
    ...
    ######################################################################
#    def n_getstate__(self):
#        return (self.input_size, self.hidden_size, self.activation)
...
#        return (self.in_gate, self.forget_gate, self.in_gate2, self.out_gate, self.internal_layers, self.initial_hidden_state, self.input_size, self.hidden_size, self.activation, self.clip_gradients,  self.is_recursive)
##        return (self.in_gate, self.forget_gate, self.in_gate2, self.out_gate, self.initial_hidden_state)
##        return (self.internal_layers, self.initial_hidden_state)
##        return self.internal_layers
##        return self.initial_hidden_state
##        return self.in_gate
##        return self.forget_gate
##        return self.out_gate
##        return self.in_gate2
#	
#    def __setstate__(self, state):
#        in_gate, forget_gate, in_gate2, out_gate, internal_layers, initial_hidden_state, input_size, hidden_size, activation, clip_gradients, is_recursive = state
##        in_gate, forget_gate, in_gate2, out_gate, initial_hidden_state = state
##        internal_layers, initial_hidden_state = state
##        internal_layers = state
##        initial_hidden_state = state
##        in_gate = state
...
#        self.in_gate2    = Layer(self.input_size + self.hidden_size, self.hidden_size, T.nnet.sigmoid, self.clip_gradients)
        # output modulation
...
class StackedCells(object):
    ...
    def __init__(self, input_size, celltype=RNN, layers=None,
        ...
        self.id = np.random.randint(0, 1024)
