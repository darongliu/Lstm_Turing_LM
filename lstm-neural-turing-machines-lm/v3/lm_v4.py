...
class Model:
	...
	def __init__(self, hidden_size, input_size, vocab_size, stack_size=1, celltype=LSTM):
		...
		self.turing_updates , self.turing_predict = turing_model.build(self.turing_params , hidden_size , input_size , hidden_size , vocab_size)
...
#		print "done compile"
	...
	def create_final_prediction(self, greedy=False):
		...
		#turing_result = self.turing_predict(result[-2][:,:,hidden_size:],result[-2][:,:,hidden_size:],result[-3]) 
                turing_result = self.turing_predict(result[-2][:,:,hidden_size:],result[-3],result[-2][:,:,hidden_size:])
