...
#        ][::-1]
    ...
    P.weighted_memory_init = 2 * (np.random.rand(1, weighted_mem_width) - 0.5).astype(theano.config.floatX)
    heads = [head.build(P, h, input_size, mem_width) for h in range(no_heads)]
...
#        return T.batched_dot(shift , weight[:,shift_conv])
    ...
    def build_head_curr(weight_prev, M_curr, head, input_curr):
        ...
        """
        key, beta, g, gamma = head(input_curr)

        # 3.3.1 Focusing b Content
        weight_c = T.nnet.softmax(T.batched_dot(beta , similarity(key, M_curr)))
...
#        weight_shifted = shift_convolve(weight_g, shift)#not modified
        ...
        weight_sharp = weight_g ** gamma.dimshuffle((0,'x'))
        weight_curr = weight_sharp / T.sum(weight_sharp , axis=1).dimshuffle((0,'x'))
...
def build(P, input_size, mem_width, weighted_mem_width, ctrl):
    step = build_step(
        P, ctrl, input_size, mem_width, weighted_mem_width)

    def predict(input_hidden, lstm_output_gate, weighted_mem):
        ...
        outputs , _ = theano.scan(
            ...
            sequences=time_idx,
            outputs_info = None,
            non_sequences=[lstm_output_gate,input_hidden,weighted_mem]
        )
        return outputs
    return predict
